...
function renderCourseSelectGrid() {
  ...
  if (!courses.length) {
    grid.innerHTML = '<div style="color:var(--gray-400);font-size:0.85rem">No courses available. Please contact admin.</div>';
    return;
  }
  grid.innerHTML = courses.map(c => `
    <div class="course-card-select ${selectedCourseId === c.id ? 'selected' : ''}" onclick="selectCourse('${c.id}')">
      <div class="c-icon">${c.icon || '📚'}</div>
      <div class="c-name">${c.name}</div>
      <div class="c-meta">${c.questions.length} questions</div>
    </div>`).join('');
}
...
function updateTimerDisplay() {
  ...
  document.getElementById('timer-display').style.color = danger ? '#EF4444' : '#F5A623';
  document.getElementById('timer-circle').style.stroke = danger ? '#EF4444' : '#F5A623';
}
...
function renderCoursesGrid() {
  ...
  grid.innerHTML = courses.map(c => {
    ...
    return `<div class="course-mgr-card">
      <div class="c-header">
